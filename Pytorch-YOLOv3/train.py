...
parser.add_argument("--log_interval", type=int, default=50, help="interval train loss")
parser.add_argument("--cut_num", type=int, default=200, help="cut number of batch size")
...
# Get dataloader
train_dataset = ListDataset(train_path, augment=True, multiscale=opt.multiscale_training)
...
val_dataset = ListDataset(valid_path, augment=True, multiscale=opt.multiscale_training)
    ...
    for batch_i, (_, imgs, targets) in enumerate(train_dataloader): 
        ...
        if batch_i % opt.log_interval == 0:
            print("===> Epoch[{}/{}]({}/{}): Loss: {:.4f}".format(
                epoch, opt.epochs, batch_i, opt.cut_num, loss.item()))
        
        if batch_i == opt.cut_num:
            break

    train_avg_losses = torch.mean(torch.FloatTensor(train_losses))
